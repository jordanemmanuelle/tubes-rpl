...
if (isset($_POST['konfirmasi'])) {
    ...
    if ($nama_penerima === '' || $alamat_penerima === '') {
        ...
        foreach ($cart as $item) {
            ...
            $query_detail = "INSERT INTO detail_transaksi (id_transaksi, id_produk, jumlah, harga, nama_penerima, alamat_penerima) 
                             VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($connect, $query_detail);
            mysqli_stmt_bind_param($stmt, "iiidss", $id_transaksi, $id_menu, $qty, $total_dengan_kurir, $nama_penerima, $alamat_penerima);
            if (!mysqli_stmt_execute($stmt)) {
